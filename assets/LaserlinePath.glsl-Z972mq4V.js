import{aC as o,aH as pe,aB as T,ll as ue,cY as Y,q3 as fe,q2 as ge,gz as _e,dA as Z,B9 as me,B6 as Pe,BQ as ve,z9 as f,Aq as w,Bb as be,za as j,z7 as r,db as O,y$ as J,B8 as we,A6 as S,cA as Ve,dB as m,en as $,e$ as B,BR as De,l7 as K,Ao as ee,zI as C,q0 as xe,dC as P,BS as te,jN as ie,eW as ne,e_ as Ee,eY as ye,da as Le,fi as Se,em as Ce,aD as se,pZ as Me,zb as ae,zc as re,v as le,Bf as Ae,y_ as $e,dz as N,fa as q,B7 as Re,ss as Te,fP as ze,zG as qe,Ay as Oe,z6 as W,BT as Ue,_ as u,zq as D,Bg as Ie,Bh as H,BU as je,pB as Be,BV as R,BW as Ne,fc as We,fd as He,rf as Fe,ff as Ge,fg as ke,fh as Xe,m as M,b as Qe,BX as Ye,fM as F}from"./index-C5z1JKvU.js";import{t as Ze}from"./VisualElement-fQOfrM3i.js";let G=class{constructor(e){this._renderCoordsHelper=e,this._origin=o(),this._dirty=!1,this._count=0}set vertices(e){const n=pe(3*e.length);let i=0;for(const s of e)n[i++]=s[0],n[i++]=s[1],n[i++]=s[2];this.buffers=[n]}set buffers(e){if(this._buffers=e,this._buffers.length>0){const n=this._buffers[0],i=3*Math.floor(n.length/3/2);T(this._origin,n[i],n[i+1],n[i+2])}else T(this._origin,0,0,0);this._dirty=!0}get origin(){return this._origin}draw(e){const n=this._ensureVAO(e);n!=null&&(e.bindVAO(n),e.drawArrays(ue.TRIANGLES,0,this._count))}dispose(){this._vao!=null&&this._vao.dispose()}get _layout(){return this._renderCoordsHelper.viewingMode===1?Ke:et}_ensureVAO(e){return this._buffers==null?null:(this._vao??=this._createVAO(e,this._buffers),this._ensureVertexData(this._vao,this._buffers),this._vao)}_createVAO(e,n){const i=this._createDataBuffer(n);return this._dirty=!1,new fe(e,new ge(e,_e(this._layout),i))}_ensureVertexData(e,n){if(!this._dirty)return;const i=this._createDataBuffer(n);e.buffer()?.setData(i),this._dirty=!1}_createDataBuffer(e){const n=e.reduce((g,c)=>g+k(c),0);this._count=n;const i=this._layout.createBuffer(n),s=this._origin;let a=0,l=0;const d="startUp"in i?this._setUpVectors.bind(this,i):void 0;for(const g of e){for(let c=0;c<g.length;c+=3){const L=T(X,g[c],g[c+1],g[c+2]);c===0?l=this._renderCoordsHelper.getAltitude(L):this._renderCoordsHelper.setAltitude(L,l);const _=a+2*c;d?.(c,_,g,L);const I=Z(X,L,s);if(c<g.length-3){for(let v=0;v<6;v++)i.start.setVec(_+v,I);i.extrude.setValues(_,0,-1),i.extrude.setValues(_+1,1,-1),i.extrude.setValues(_+2,1,1),i.extrude.setValues(_+3,0,-1),i.extrude.setValues(_+4,1,1),i.extrude.setValues(_+5,0,1)}if(c>0)for(let v=-6;v<0;v++)i.end.setVec(_+v,I)}a+=k(g)}return i.buffer}_setUpVectors(e,n,i,s,a){const l=this._renderCoordsHelper.worldUpAtPosition(a,Je);if(n<s.length-3)for(let d=0;d<6;d++)e.startUp.setVec(i+d,l);if(n>0)for(let d=-6;d<0;d++)e.endUp.setVec(i+d,l)}};function k(t){return 3*(2*(t.length/3-1))}const Je=o(),X=o(),Ke=Y().vec3f("start").vec3f("end").vec2f("extrude").vec3f("startUp").vec3f("endUp"),et=Y().vec3f("start").vec3f("end").vec2f("extrude");function oe(t,e){const n=t.fragment;n.include(me),t.include(Pe),n.include(ve),n.uniforms.add(new f("globalAlpha",i=>i.globalAlpha),new w("glowColor",i=>i.glowColor),new f("glowWidth",(i,s)=>i.glowWidth*s.camera.pixelRatio),new f("glowFalloff",i=>i.glowFalloff),new w("innerColor",i=>i.innerColor),new f("innerWidth",(i,s)=>i.innerWidth*s.camera.pixelRatio),new be("depthMap",i=>i.depth?.attachment),new j("normalMap",i=>i.normals)),n.code.add(r`vec4 premultipliedColor(vec3 rgb, float alpha) {
